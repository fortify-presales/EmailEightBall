...
Write-Host ************************************************************
& mvn clean
& sourceanalyzer -Dcom.fortify.sca.ProjectRoot=.fortify -b eightball -clean

# Compile the application
Write-Host ************************************************************
Write-Host Re-compiling application in debug mode...
Write-Host ************************************************************
& mvn -DskipTests '-Dmaven.compiler.debuglevel="lines,vars,source"' verify package
...
Write-Host ************************************************************
& sourceanalyzer '-Dcom.fortify.sca.ProjectRoot=.fortify' -b eightball -findbugs -cp $ClassPath  -java-build-dir "target/classes" `
	-build-project "Email Eight Ball" -build-version "v1.0" -build-label "SNAPSHOT" -scan -f target\eightball.fpr
...
Write-Host ************************************************************
& ReportGenerator '-Dcom.fortify.sca.ProjectRoot=.fortify' -user "Demo User" -format pdf -f target\eightball.pdf -source target\eightball.fpr
...
Write-Host ************************************************************
if ($UploadToSSC) {
	& fortifyclient uploadFPR -file target\eightball.fpr -url $FortifySSCUrl -authtoken $Env:SSC_AUTH_TOKEN -applicationVersionID $FortifySSCAppVersionId
}	
